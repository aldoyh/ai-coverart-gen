...
const TitleInput: React.FC<TitleInputProps> = ({ 
  ...
  const aspectRatios = [
    { value: '16:9', label: labels.ratios['16:9'] },
    { value: '1:1', label: labels.ratios['1:1'] },
    { value: '9:16', label: labels.ratios['9:16'] },
  ];
    ...
    <form onSubmit={onSubmit} className="w-full">
      <div className="flex flex-col sm:flex-row items-center gap-4 w-full">
        <input
          ...
          disabled={isLoading}
          className="w-full flex-grow px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-300 disabled:opacity-50 rtl:text-right ltr:text-left"
          aria-label="YouTube or episode title"
        />
        <button
          type="submit"
          disabled={isLoading || !value}
          className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 font-semibold text-white bg-gradient-to-r from-purple-600 to-pink-600 rounded-lg shadow-lg hover:from-purple-700 hover:to-pink-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-purple-500 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-105 disabled:hover:scale-100 whitespace-nowrap"
        >
          {isLoading ? (
            <>
            ...
            <>
              <MagicWandIcon className="w-5 h-5 rtl:ml-2 rtl:-mr-1" />
              {labels.generate}
            </>
          )}
        </button>
      ...
      <div className="flex items-center justify-center gap-2 sm:gap-4 mt-4">
        <span className="text-sm font-medium text-gray-400 shrink-0">{labels.aspectRatio}:</span>
